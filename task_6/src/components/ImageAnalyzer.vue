<template>
      ...
      <div v-if="imageSrc">
        ...
        <div>
          ...
          <svg width="300" height="150" style="background: #f8f8f8;">
            <line 
              v-for="(value, index) in displayedHistogram" 
              :key="index"
              :x1="300 - index" 
              y1="150" 
              :x2="300 - index" 
  ...
  <script>
  // Простая функция для преобразования RGB в CMYK
  ...
  export default {
    ...
    computed: {
      displayedHistogram() {
        ...
        // Return sum of all channels for 'all'
        return this.histogramData.c.map((c, i) => c + this.histogramData.m[i] + this.histogramData.y[i] + this.histogramData.k[i]);
      }
    },
    methods: {
      onFileChange(e) {
